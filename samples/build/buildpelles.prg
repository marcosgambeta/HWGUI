...
FUNCTION BuildBat()
   ...
   fwrite(oArq, vHarbour + "\BIN\HARBOUR " + voPrgMain + " -o" + oName + ;
      " -i" + vPelles + "\INCLUDE;" + vHarbour + "\INCLUDE;" + vHwGUI + "\INCLUDE" + ;
      iif(!empty(voIncFolder), ";", "") + voIncFolder + " " + voPrgFlag + " -n -q0 -es2 -gc0" + CRF)

   IF Len(voPrgFiles)>0
   for i := 1 to Len(voPrgFiles)
      IF !empty(voPrgFiles[i])
         oName := Substr(voPrgFiles[i], 1, Len(voPrgFiles[i]) - 4)
         fwrite(oArq, vHarbour + "\BIN\HARBOUR " + voPrgFiles[i] + " -o" + oName + ;
            " -i" + vPelles + "\INCLUDE;" + vHarbour + "\INCLUDE;" + vHwGUI + ;
            "\INCLUDE" + iif(!empty(voIncFolder), ";", "") + voIncFolder + " " + ;
            voPrgFlag + " -n -q0 -es2 -gc0" + CRF)
      ENDIF
   Next
   ENDIF
