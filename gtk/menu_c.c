/*
 * $Id: menu_c.c 2047 2013-05-24 10:44:30Z alkresin $
 *
...
HB_FUNC(HWG__CHECKMENUITEM)
{
  ...
  g_signal_handlers_block_matched((gpointer)check_menu_item, G_SIGNAL_MATCH_FUNC, 0, 0, 0, G_CALLBACK(cb_signal), 0);
  gtk_check_menu_item_set_active(check_menu_item, (HB_ISNIL(2)) ? 1 : hb_parl(2));
  g_signal_handlers_unblock_matched((gpointer)check_menu_item, G_SIGNAL_MATCH_FUNC, 0, 0, 0, G_CALLBACK(cb_signal), 0);
}
...
HB_FUNC(HWG__ADDACCELERATOR)
{
  ...
  gtk_widget_add_accelerator((GtkWidget *)HB_PARHANDLE(2), "activate", (GtkAccelGroup *)HB_PARHANDLE(1),
