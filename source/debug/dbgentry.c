/*
 * $Id: dbgentry.c 2076 2013-06-13 15:37:33Z druzus $
 */
...
#define ARRAY_ADD(type, array, length)                                                             \
  ((++length == 1) ? (array = (type *)hb_xgrab(sizeof(type)))                                      \
                   : ((array = (type *)hb_xrealloc(array, sizeof(type) * length)) + length - 1))
...
static void hb_dbgAddVar(int *nVars, HB_VARINFO **aVars, const char *szName, char cType, int nIndex,
                         int nFrame, PHB_ITEM pFrame);
...
static PHB_ITEM hb_dbgActivateModuleArray(void)
{
  ...
  for (i = 0; i < s_common.nModules; i++)
  {
    ...
    item = hb_dbgActivateVarArray(s_common.aModules[i].nStatics, s_common.aModules[i].aStatics);
    hb_arraySet(pModule, 2, item);
    hb_itemRelease(item);

    item = hb_dbgActivateVarArray(s_common.aModules[i].nGlobals, s_common.aModules[i].aGlobals);
...
void hb_dbgEntry(int nMode, int nLine, const char *szName, int nIndex, PHB_ITEM pFrame)
{
  ...
  case HB_DBG_SHOWLINE: {
    ...
    if (hb_dbgIsBreakPoint(info, pTop->szModule, nLine) || hb_dbg_InvokeDebug(HB_FALSE) ||
...
static void hb_dbgAddStatic(HB_DEBUGINFO *info, const char *szName, int nIndex, PHB_ITEM pFrame)
{
  if (info->bInitGlobals)
  {
    ...
    HB_DBGCOMMON_LOCK
    module = &s_common.aModules[s_common.nModules - 1];
    hb_dbgAddVar(&module->nExternGlobals, &module->aExternGlobals, szName, 'G', nIndex,
                 hb_dbg_vmVarGCount(), NULL);
    HB_DBGCOMMON_UNLOCK
  }
    ...
    for (i = 1; i <= nItemLen; i++)
    {
      ...
      for (j = 1; j <= nLinesLen; j++)
      {
        ...
        if (!strcmp(hb_arrayGetCPtr(pLines, 1), szModule))
        {
          ...
          int nMax = HB_MAX(nNewMin + (nNewLen << 3) - 1, nOrigMin + (nOrigLen << 3) - 1);
...
static int hb_dbgEvalSubstituteVar(HB_WATCHPOINT *watch, char *szWord, int nStart, int nLen)
{
  ...
  t[nStart + 8] = ']';
  hb_strncpy(t + nStart + 9, watch->szExpr + nStart + nLen,
             (HB_SIZE)strlen(watch->szExpr) - nLen - nStart);
...
static PHB_ITEM hb_dbgEvalMakeBlock(HB_WATCHPOINT *watch)
{
  ...
  while (watch->szExpr[i])
  {
    ...
    if (c == '.')
    {
      if (watch->szExpr[i + 1] && strchr("TtFf", watch->szExpr[i + 1]) &&
          watch->szExpr[i + 2] == '.')
      {
        i += 3;
      }
    ...
    if (c == ':' ||
        (c == '-' && watch->szExpr[i + 1] == '>' && IS_IDENT_START(watch->szExpr[i + 2])))
    {
