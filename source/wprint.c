/*
 * $Id: wprint.c 1971 2012-12-26 08:38:02Z alkresin $
 *
...
HB_FUNC(HWG_GETDEFAULTPRINTER)
{
  ...
  if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) // Windows 98
  {
    ...
    EnumPrinters(PRINTER_ENUM_DEFAULT, NULL, 5, (LPBYTE)pinfo5, dwNeeded, &dwNeeded, &dwReturned);
...
HB_FUNC(HWG_GETPRINTERS)
{
  ...
  if (GetVersion() & 0x80000000) // Windows 98
  {
    ...
    if (dwNeeded)
    {
      ...
      EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 5, pBuffer, dwNeeded, &dwNeeded, &dwReturned);
    }
  }
  else // Windows NT
  {
    ...
    if (dwNeeded)
    {
      ...
      EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 4, pBuffer, dwNeeded, &dwNeeded, &dwReturned);
    }
  }
  if (dwReturned)
  {
...
HB_FUNC(HWG_SETPRINTERMODE)
{
  ...
  HANDLE hPrinter = (HB_ISNIL(2)) ? (HANDLE)NULL : (HANDLE)HB_PARHANDLE(2);
  long int nSize;
  PDEVMODE pdm;
...
HB_FUNC(HWG_CREATEENHMETAFILE)
{
  ...
  // sprintf(cres, "%d %d %d %d %d %d %d %d", iWidthMM, iHeightMM, iWidthPels, iHeightPels, rc.left,
  // rc.top, rc.right, rc.bottom); MessageBox(GetActiveWindow(), cres, "", MB_OK |
  // MB_ICONINFORMATION);
    ...
    if (OpenPrinter((LPTSTR)lpPrinterName, &hPrinter, NULL))
    {
      ...
      if (lSize > 0)
      {
                                           ...
                                           DM_OUT_BUFFER) == IDOK) // Get the current settings
        {
          ...
          /* NOTES:
             For unknown reasons, Windows98/ME returns IDCANCEL if user clicks OK without changing
