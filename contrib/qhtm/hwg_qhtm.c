...
static BOOL s_qhtmInit(LPCTSTR lpLibname)
{
  if (!s_hQhtmDll)
  {
    ...
    if (s_hQhtmDll)
    {
      QHTM_INITIALIZE pFunc = (QHTM_INITIALIZE)(void *)GetProcAddress(s_hQhtmDll, "QHTM_Initialize");
      if (pFunc)
...
HB_FUNC(QHTM_MESSAGE)
{
  if (s_qhtmInit(NULL))
  {
    QHTM_MESSAGEBOX pFunc = (QHTM_MESSAGEBOX)(void *)GetProcAddress(s_hQhtmDll, "QHTM_MessageBox");
    ...
    if (pFunc)
    {
      ...
      pFunc(GetActiveWindow(), HB_PARSTR(1, &hText, NULL), HB_PARSTRDEF(2, &hTitle, NULL), uType);
      hb_strfree(hText);
      hb_strfree(hTitle);
    }
  }
...
HB_FUNC(QHTM_FORMCALLBACK)
{
  if (s_qhtmInit(NULL))
  {
    hb_retl(SendMessage((HWND)hb_parnl(1), QHTM_SET_OPTION, (WPARAM)QHTM_OPT_SET_FORM_SUBMIT_CALLBACK,
                        (LPARAM)FormCallback));
  }
  else
    hb_retl(0);
...
HB_FUNC(QHTM_ENABLECOOLTIPS)
{
  if (s_qhtmInit(NULL))
  {
    QHTM_ENABLECOOLTIPS pFunc = (QHTM_ENABLECOOLTIPS)(void *)GetProcAddress(s_hQhtmDll, "QHTM_EnableCooltips");
    if (pFunc)
      pFunc();
    else
      hb_retl(0);
...
HB_FUNC(QHTM_SETHTMLBUTTON)
{
  if (s_qhtmInit(NULL))
  {
    QHTM_SETHTMLBUTTON pFunc = (QHTM_SETHTMLBUTTON)(void *)GetProcAddress(s_hQhtmDll, "QHTM_SetHTMLButton");
    if (pFunc)
...
HB_FUNC(QHTM_PRINTCREATECONTEXT)
{
  if (s_qhtmInit(NULL))
  {
    QHTM_PRINTCREATECONTEXT pFunc =
        (QHTM_PRINTCREATECONTEXT)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintCreateContext");
...
HB_FUNC(QHTM_PRINTSETTEXT)
{
  if (s_qhtmInit(NULL))
  {
    ...
    QHTM_PRINTSETTEXT pFunc = (QHTM_PRINTSETTEXT)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintSetText");
    hb_retl(pFunc((QHTMCONTEXT)hb_parnl(1), HB_PARSTR(2, &hText, NULL)));
    hb_strfree(hText);
  }
  else
    hb_retl(0);
...
HB_FUNC(QHTM_PRINTSETTEXTFILE)
{
  if (s_qhtmInit(NULL))
  {
    ...
    QHTM_PRINTSETTEXTFILE pFunc = (QHTM_PRINTSETTEXTFILE)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintSetTextFile");
    hb_retl(pFunc((QHTMCONTEXT)hb_parnl(1), HB_PARSTR(2, &hText, NULL)));
    hb_strfree(hText);
  }
  else
...
HB_FUNC(QHTM_PRINTSETTEXTRESOURCE)
{
  if (s_qhtmInit(NULL))
  {
    ...
    QHTM_PRINTSETTEXTRESOURCE pFunc =
        (QHTM_PRINTSETTEXTRESOURCE)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintSetTextResource");
    hb_retl(pFunc((QHTMCONTEXT)hb_parnl(1), GetModuleHandle(NULL), HB_PARSTR(2, &hText, NULL)));
    hb_strfree(hText);
  }
  else
    hb_retl(0);
...
HB_FUNC(QHTM_PRINTLAYOUT)
{
  if (s_qhtmInit(NULL))
  {
    ...
    RECT rcPage;
    int nNumberOfPages;
    QHTM_PRINTLAYOUT pFunc = (QHTM_PRINTLAYOUT)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintLayout");
...
HB_FUNC(QHTM_PRINTPAGE)
{
  if (s_qhtmInit(NULL))
  {
    ...
    QHTMCONTEXT qhtmCtx = (QHTMCONTEXT)hb_parnl(2);
    RECT rcPage;
    QHTM_PRINTPAGE pFunc = (QHTM_PRINTPAGE)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintPage");
...
HB_FUNC(QHTM_PRINTDESTROYCONTEXT)
{
  if (s_qhtmInit(NULL))
  {
    QHTM_PRINTDESTROYCONTEXT pFunc =
        (QHTM_PRINTDESTROYCONTEXT)(void *)GetProcAddress(s_hQhtmDll, "QHTM_PrintDestroyContext");
